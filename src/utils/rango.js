...
export function combinarFacturas(invoices) {
  ...
  // Desglose ruta×peso combinado — EXCLUSIVO del Simulador (campo `simuladorDesglose`,
  // aislado de los cálculos). Se suman cantidad e ingreso por ciudad×ruta×rango; el
  // precio típico se toma de la factura que aportó MÁS paquetes a ese tramo (aprox. de
  // la moda combinada). Se lee el campo nuevo con respaldo al antiguo `resumenRutaPeso`.
  const rpMap = {}
  for (const inv of invoices) {
    for (const x of (inv.simuladorDesglose || inv.resumenRutaPeso || [])) {
  ...
  const simuladorDesglose = Object.values(rpMap).map(({ _max, ...r }) => ({ ...r, ingreso: Math.round(r.ingreso * 100) / 100 }))
