...
export function useColeccion(nombre, filtros = []) {
  const { tenantId } = useBulkAuth()
  const [datos, setDatos] = useState([])
  const [cargando, setCargando] = useState(true)
  // clave estable de filtros para no re-suscribir en cada render
  const clave = JSON.stringify(filtros)
  useEffect(() => {
    if (!tenantId) { setDatos([]); setCargando(false); return }
    setCargando(true)
    const off = suscribir(nombre, tenantId, (d) => { setDatos(d); setCargando(false) }, filtros)
    return off
