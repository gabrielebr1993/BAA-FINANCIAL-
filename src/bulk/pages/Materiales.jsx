  ...
  return (
    <div>
      ...
      <Card className="mb-4 p-4">
        <h3 className="m-0 mb-3 text-sm font-bold text-brand-navy dark:text-slate-100">Nuevo material</h3>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Input placeholder="Nombre (ej. Grava)" value={f.nombre} onChange={set('nombre')} />
          <Select value={f.unidad} onChange={set('unidad')}>{UNIDADES.map((u) => <option key={u} value={u}>{u}</option>)}</Select>
          <div>
            <div className="mb-1 text-[11px] uppercase text-slate-400">Precio por {f.unidad}</div>
            <Input type="number" step="0.01" placeholder="0.00" value={f.precio} onChange={set('precio')} />
          </div>
          <div className="flex items-end"><Boton variant="gold" onClick={agregar} disabled={!f.nombre.trim()} className="w-full justify-center"><Plus size={16} /> Agregar</Boton></div>
        </div>
        <p className="mt-2 text-[11px] text-slate-400">El precio es la referencia del material. El precio final al cliente puede ajustarse por planta/acuerdo en el motor de tarifas y en el trabajo.</p>
      </Card>
          ...
          {materiales.slice().sort((a, b) => (a.nombre || '').localeCompare(b.nombre || '')).map((m) => (
            <Card key={m.id} className="p-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-brand-navy dark:text-slate-100">{m.nombre}</span>
                <button onClick={() => toggle(m)}><Badge color={m.activo === false ? 'slate' : 'green'}>{m.activo === false ? 'Inactivo' : 'Activo'}</Badge></button>
                <button onClick={() => window.confirm(`¿Eliminar "${m.nombre}"?`) && eliminar('materials', m.id)} className="ml-auto text-rose-400 hover:text-rose-600"><Trash2 size={14} /></button>
              </div>
              <div className="mt-2 flex items-center gap-2">
                <span className="text-xs text-slate-400">Precio por {m.unidad || 'ton'}:</span>
