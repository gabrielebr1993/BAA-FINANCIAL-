...
export default function Facturacion() {
  ...
  return (
    <div>
      <PageTitle>{t('Facturación')}</PageTitle>
      {msg && <Aviso tipo={msg.tipo} className="mb-3">{msg.txt}</Aviso>}

      <Card className="mb-4 p-4">
        <h3 className="m-0 mb-3 text-sm font-bold text-brand-navy dark:text-slate-100">{t('Nueva factura')}</h3>
      ...
      <Card className="p-4">
        ...
        {facturas.length === 0 ? <p className="text-sm text-slate-400">{t('Aún no hay facturas.')}</p> : (
          <Tabla
            columns={[{ key: 'numero', label: t('Factura') }, { key: 'clienteNombre', label: t('Cliente') }, { key: 'periodo', label: t('Periodo') }, { key: 'toneladas', label: t('Ton'), align: 'right' }, { key: 'total', label: t('Total'), align: 'right' }, { key: 'estado', label: t('Estado'), align: 'center' }, { key: 'acciones', label: '', align: 'right' }]}
