...
export default function Incidencias() {
  ...
  return (
    <div>
      <PageTitle>{t('Centro de incidencias')}</PageTitle>
      {abiertas.length > 0 && <Aviso tipo="warn" className="mb-3">{abiertas.length} {t('incidencia(s) sin resolver.')}</Aviso>}

      <Card className="mb-4 p-4">
        <h3 className="m-0 mb-3 text-sm font-bold text-brand-navy dark:text-slate-100">{t('Reportar incidencia')}</h3>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Select value={f.tipo} onChange={set('tipo')}>{TIPOS.map((tp) => <option key={tp} value={tp}>{t(tp)}</option>)}</Select>
          <Input placeholder={t('Orden (opcional)')} value={f.orden} onChange={set('orden')} />
          <Input placeholder={t('Responsable')} value={f.responsable} onChange={set('responsable')} />
          <label className="flex cursor-pointer items-center gap-2 rounded-xl border border-dashed border-slate-300 px-3 text-sm text-slate-500 dark:border-slate-600"><Camera size={16} /> {f.foto ? t('Foto ✓') : t('Foto')}<input type="file" accept="image/*" onChange={subirFoto} className="hidden" /></label>
        </div>
        <Input placeholder={t('Descripción de lo ocurrido…')} value={f.descripcion} onChange={set('descripcion')} className="mt-3" />
        <div className="mt-3"><Boton variant="gold" onClick={crearInc} disabled={!f.descripcion.trim()}><Plus size={16} /> {t('Registrar')}</Boton></div>
      </Card>

      {incidencias.length === 0 ? <EstadoVacio titulo={t('Sin incidencias')} texto={t('Registra accidentes, retrasos o daños para darles seguimiento.')} mostrarBoton={false} /> : (
        <div className="space-y-3">
          {incidencias.slice().sort((a, b) => (b.ts || '').localeCompare(a.ts || '')).map((inc) => (
            <Card key={inc.id} className="p-4">
              <div className="flex flex-wrap items-center gap-2">
                <AlertTriangle size={16} className="text-amber-500" />
                <span className="font-semibold capitalize text-brand-navy dark:text-slate-100">{t(inc.tipo)}</span>
                {inc.orden && <Badge color="slate">{inc.orden}</Badge>}
                <Badge color={ESTADOS[inc.estado]}>{t(inc.estado)}</Badge>
              ...
              {inc.responsable && <div className="text-xs text-slate-400">{t('Responsable:')} {inc.responsable}</div>}
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <Input className="flex-1" placeholder={t('Agregar nota de seguimiento…')} value={nota[inc.id] || ''} onChange={(e) => setNota((s) => ({ ...s, [inc.id]: e.target.value }))} onKeyDown={(e) => e.key === 'Enter' && agregarNota(inc)} />
                ...
                {inc.estado !== 'resuelta' && (
                  <Select value={inc.estado} onChange={(e) => cambiarEstado(inc, e.target.value)} className="py-1 text-xs">
                    {Object.keys(ESTADOS).map((s) => <option key={s} value={s}>{t(s)}</option>)}
                  </Select>
                )}
              </div>
            </Card>
