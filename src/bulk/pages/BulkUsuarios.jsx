  ...
  return (
    <div>
      ...
      <Card className="mb-4 p-4">
        <h3 className="m-0 mb-3 text-sm font-bold text-brand-navy dark:text-slate-100">Nuevo usuario</h3>
      ...
      <Card className="p-4">
        <Tabla
          columns={[{ key: 'nombre', label: 'Nombre' }, { key: 'email', label: 'Correo' }, { key: 'rol', label: 'Rol' }, { key: 'estado', label: 'Estado', align: 'center' }, { key: 'acciones', label: '', align: 'right' }]}
          rows={usuarios.slice().sort((a, b) => (a.nombre || '').localeCompare(b.nombre || '')).map((u) => ({ ...u, _key: u.id }))}
          emptyText="Sin usuarios."
          renderCell={(row, key) => {
            if (key === 'rol') return <Badge color={row.rol === BULK_ROLES.SUPER_ADMIN ? 'gold' : 'navy'}>{BULK_ROLES_LABEL[row.rol] || row.rol}</Badge>
            ...
            if (key === 'acciones') return row.rol === BULK_ROLES.SUPER_ADMIN ? <span className="inline-flex items-center gap-1 text-xs text-slate-400"><ShieldCheck size={13} /> protegido</span> : <Boton variant="danger" onClick={() => borrar(row)} className="px-2.5 py-1 text-xs"><Trash2 size={13} /></Boton>
            return row[key]
          }}
        />
      </Card>
