...
export default function Documentos() {
  ...
  return (
    <div>
      <PageTitle>Documentos y vencimientos</PageTitle>
      {alertas.length > 0 && <Aviso tipo="warn" className="mb-3">{alertas.length} documento(s) vencidos o por vencer en 30 días. Renuévalos.</Aviso>}

      <Card className="mb-4 p-4">
        <h3 className="m-0 mb-3 text-sm font-bold text-brand-navy dark:text-slate-100">Nuevo documento</h3>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Select value={f.carrierId} onChange={set('carrierId')}><option value="">— Transportista —</option>{carriers.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}</Select>
          <Select value={f.tipo} onChange={set('tipo')}>{TIPOS.map((t) => <option key={t} value={t}>{t}</option>)}</Select>
          <Input placeholder="Número / folio" value={f.numero} onChange={set('numero')} />
          <div><div className="mb-1 text-[11px] uppercase text-slate-400">Vence</div><Input type="date" value={f.vence} onChange={set('vence')} /></div>
        </div>
        <div className="mt-3"><Boton variant="gold" onClick={agregar} disabled={!f.carrierId || !f.vence}><Plus size={16} /> Agregar</Boton></div>
      </Card>

      {enriquecidos.length === 0 ? <EstadoVacio titulo="Sin documentos" texto="Registra seguros, licencias, DOT, MC, inspecciones y sus vencimientos." mostrarBoton={false} /> : (
        <Card className="p-4">
          <div className="space-y-2">
            {enriquecidos.map((d) => (
              <div key={d.id} className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-100 p-3 dark:border-slate-700/50">
                ...
                <span className="font-semibold text-brand-navy dark:text-slate-100">{d.tipo}</span>
                {d.numero && <span className="text-xs text-slate-400">#{d.numero}</span>}
                <Badge color="navy">{nombreCarrier(d.carrierId)}</Badge>
                <span className="text-xs text-slate-400">vence {d.vence}</span>
                <Badge color={COLOR[d.estado]}>{LABEL[d.estado]}{d.dias != null && d.estado !== 'ok' ? ` · ${d.dias < 0 ? `hace ${-d.dias}d` : `en ${d.dias}d`}` : ''}</Badge>
                <button onClick={() => window.confirm('¿Eliminar documento?') && eliminar('documents', d.id)} className="ml-auto text-rose-400 hover:text-rose-600"><Trash2 size={14} /></button>
              </div>
            ))}
          </div>
        </Card>
