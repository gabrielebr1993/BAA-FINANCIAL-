...
export default function CampanaNotificaciones({ notifs = [], claveLS = 'bulk_notif_leidas', invertido = false }) {
  ...
  const abrir = (n) => { marcarLeida(n.id); setAbierto(false); if (n.link) navigate(n.link) }

  const btnClase = invertido
    ? 'relative grid h-9 w-9 place-items-center rounded-lg text-slate-300 hover:bg-white/10'
    : 'relative grid h-9 w-9 place-items-center rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800'

  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setAbierto((v) => !v)} title={t('Notificaciones')} className={btnClase}>
        <Bell size={17} />
        {pendientes.length > 0 && (
      ...
      {abierto && (
        <div className="absolute right-0 z-50 mt-2 w-[min(92vw,360px)] overflow-hidden rounded-xl border border-slate-200 bg-white text-left shadow-2xl dark:border-slate-700 dark:bg-slate-900">
