  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950">
      <header className="flex items-center gap-2 bg-slate-900 px-4 py-3 text-white">
        ...
        <div><div className="text-sm font-bold">{usuario?.nombre}</div><div className="text-[11px] text-slate-400">Portal del cliente</div></div>
      ...
      <main className="mx-auto max-w-5xl p-4">
        {!usuario?.clienteId ? <EstadoVacio titulo="Cuenta no vinculada" texto="Tu usuario aún no está ligado a un cliente. Pídele al administrador que lo asigne." mostrarBoton={false} /> : (
          <>
            <div className="mb-4 flex flex-wrap gap-3">
              <KPI label="Órdenes" value={stats.total} icon={ClipboardList} accent="navy" />
              <KPI label="Entregadas" value={stats.entregadas} icon={ClipboardList} accent="green" />
              <KPI label="Toneladas entregadas" value={Math.round(stats.ton)} icon={Weight} accent="gold" />
              <KPI label="Gasto total" value={money(stats.gasto)} icon={DollarSign} accent="blue" />
            </div>
            <div className="mb-4 grid gap-3 sm:grid-cols-3">
              <Card className="p-4"><div className="text-xs text-slate-400">Gasto hoy</div><div className="text-xl font-bold text-brand-navy dark:text-slate-100">{money(stats.hoy)}</div></Card>
              <Card className="p-4"><div className="text-xs text-slate-400">Gasto esta semana</div><div className="text-xl font-bold text-brand-navy dark:text-slate-100">{money(stats.semana)}</div></Card>
              <Card className="p-4"><div className="text-xs text-slate-400">Gasto este mes</div><div className="text-xl font-bold text-brand-navy dark:text-slate-100">{money(stats.mes)}</div></Card>
            </div>

            <Card className="mb-4 p-4">
              <h3 className="m-0 mb-3 text-base font-bold text-brand-navy dark:text-slate-100">Costos por material</h3>
              {stats.porMaterial.length === 0 ? <p className="text-sm text-slate-400">Sin entregas todavía.</p> : (
                <Tabla columns={[{ key: 'material', label: 'Material' }, { key: 'ton', label: 'Toneladas', align: 'right' }, { key: 'gasto', label: 'Gasto', align: 'right' }]}
            ...
            <Card className="p-4">
              <h3 className="m-0 mb-3 text-base font-bold text-brand-navy dark:text-slate-100">Órdenes recientes</h3>
              {ordenes.length === 0 ? <p className="text-sm text-slate-400">Aún no hay órdenes.</p> : (
                <Tabla columns={[{ key: 'numero', label: 'Orden' }, { key: 'material', label: 'Material' }, { key: 'ton', label: 'Ton', align: 'right' }, { key: 'precioCliente', label: 'Precio', align: 'right' }, { key: 'estado', label: 'Estado', align: 'center' }]}
                  ...
                  renderCell={(o, k) => {
                    ...
                    if (k === 'estado') return <Badge color={ENTREGADAS.includes(o.estado) ? 'green' : 'navy'}>{ORDEN_ESTADO_LABEL[o.estado]}</Badge>
                    return o[k]
                  }} />
              )}
            </Card>
