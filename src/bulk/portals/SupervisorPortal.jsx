...
export default function SupervisorPortal() {
  ...
  const liberar = async (orden) => {
    ...
    setMsg({ tipo: 'ok', txt: `Orden ${orden.numero} liberada. El chofer ya puede tomar otra carga.` })
  }

  const liberarPorCodigo = async () => {
    setMsg(null)
    const o = pendientes.find((x) => (x.numero || '').toLowerCase() === codigo.trim().toLowerCase())
    if (!o) { setMsg({ tipo: 'error', txt: 'No hay una orden entregada con ese código esperando liberación.' }); return }
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col bg-slate-100 dark:bg-slate-950">
      <header className="flex items-center gap-2 bg-slate-900 px-4 py-3 text-white">
        ...
        <div><div className="text-sm font-bold">{usuario?.nombre}</div><div className="text-[11px] text-slate-400">Supervisor de planta</div></div>
      ...
      <main className="flex-1 overflow-y-auto p-3">
        ...
        <Card className="mb-4 p-4 text-center">
          ...
          <div className="mt-1 text-sm font-semibold text-brand-navy dark:text-slate-100">Escanear / escribir código del chofer</div>
          <div className="mt-3 flex gap-2">
            <Input placeholder="Ej. J1AB2-0001" value={codigo} onChange={(e) => setCodigo(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && liberarPorCodigo()} className="flex-1" />
            <Boton variant="gold" onClick={liberarPorCodigo} disabled={!codigo.trim()}>Liberar</Boton>
          </div>
          <p className="mt-2 text-[11px] text-slate-400">La orden solo se libera tras verificar el código. (Fase avanzada: cámara para escanear el QR.)</p>
        </Card>

        <h3 className="mb-2 text-sm font-bold text-brand-navy dark:text-slate-100">Esperando liberación ({pendientes.length})</h3>
        {pendientes.length === 0 ? <p className="text-sm text-slate-400">Ninguna orden esperando. Cuando un chofer entregue, aparecerá aquí.</p> : (
          pendientes.map((o) => (
            <Card key={o.id} className="mb-2 p-3">
              <div className="flex items-center gap-2">
                ...
                <Boton variant="success" onClick={() => liberar(o)} className="ml-auto px-3 py-1 text-xs"><CheckCircle2 size={14} /> Liberar</Boton>
              </div>
              <div className="mt-1 text-xs text-slate-400">{o.material} · chofer: {o.choferNombre || '—'}</div>
            </Card>
          ))
        )}
      </main>
