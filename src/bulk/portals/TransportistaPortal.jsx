    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950">
      ...
      <main className="mx-auto max-w-5xl p-4">
        ...
        <div className="mb-4 inline-flex overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700">
          {[{ k: 'ordenes', l: 'Órdenes' }, { k: 'choferes', l: 'Mis choferes' }, { k: 'equipos', l: 'Equipos' }].map((t) => (
        ...
        {tab === 'ordenes' && (
          ordenes.length === 0 ? <EstadoVacio texto="Cuando el dispatcher te asigne órdenes, aparecerán aquí para que asignes tus choferes." mostrarBoton={false} /> : (
              ...
              {ordenes.slice().sort((a, b) => (b.numero || '').localeCompare(a.numero || '')).map((o) => {
                ...
                return (
                  <Card key={o.id} className="p-3">
                    ...
                    {o.choferNombre && <div className="mt-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400">Chofer: {o.choferNombre}</div>}
                    {!FINAL.includes(o.estado) && (
                      choferes.length > 0 ? (
                        <Select className="mt-2 w-full py-1 text-xs" value={o.choferId || ''} onChange={(e) => e.target.value && asignarChofer(o, e.target.value)}>
                          <option value="">{o.choferId ? 'Cambiar chofer…' : 'Asignar chofer…'}</option>
                          {choferes.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                        </Select>
                      ) : <div className="mt-2 text-[11px] text-slate-400">El administrador aún no te ha registrado choferes.</div>
                    )}
                  </Card>
                )
              })}
        ...
        {tab === 'choferes' && (
          <>
            <Aviso tipo="info" className="mb-3">Tus choferes los da de alta y transfiere el administrador. Aquí los ves y los asignas a tus órdenes (un chofer puede ir en varias).</Aviso>
            {choferes.length === 0 ? <EstadoVacio titulo="Sin choferes" texto="Pídele al administrador que registre tus choferes." mostrarBoton={false} /> : (
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {choferes.map((c) => (
                  <Card key={c.id} className="p-3">
                    ...
                    <div className="mt-1"><Badge color={c.activo === false ? 'slate' : 'green'}>{c.activo === false ? 'Inactivo' : 'Activo'}</Badge></div>
                  </Card>
                ))}
              </div>
            )}
