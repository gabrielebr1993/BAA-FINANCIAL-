...
export default function FotoPerfil({ url, alt = '', icon: Icon, grande = true }) {
  ...
  if (!url) {
    return (
      <div className={`grid ${dim} flex-shrink-0 place-items-center rounded-2xl bg-brand-navy text-brand-gold`}>
  ...
  return (
    <>
      <button onClick={() => setZoom(true)} title="Ampliar foto" className={`group relative ${dim} flex-shrink-0 overflow-hidden rounded-2xl ring-1 ring-slate-200 dark:ring-slate-700`}>
