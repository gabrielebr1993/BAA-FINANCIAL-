        ...
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            {empresa.claimModo === 'real' ? (
              <div className="flex h-[38px] w-32 items-center justify-center rounded-lg bg-slate-100 px-2 text-[11px] text-slate-400 dark:bg-slate-800">Se cobra lo de Gofo</div>
            ) : (
          ...
          <div>
            ...
            {empresa.claimModo === 'real' ? (
              <div className="flex h-[38px] w-40 items-center justify-center rounded-lg bg-slate-100 px-2 text-[11px] text-slate-400 dark:bg-slate-800">Se cobra lo de Gofo</div>
            ) : (
              ...
              {ciudadesConCodigo.map((c) => {
                ...
                <tr key={c.codigo} className="border-t border-slate-100 dark:border-slate-700/50">
                  ...
                  <td className="px-3 py-2 text-right">
                    {real ? <span className="text-xs italic text-slate-400">Gofo</span>
                          : <Input className="w-28 text-right" type="number" step="0.01" min="0" value={valCiudad(c.codigo, 'claimFee')} onChange={(e) => setCiudad(c.codigo, 'claimFee', e.target.value)} placeholder={`${empClaim}`} />}
                  </td>
                  <td className="px-3 py-2 text-right">
                    {real ? <span className="text-xs italic text-slate-400">Gofo</span>
                          : <Input className="w-28 text-right" type="number" step="0.01" min="0" value={valCiudad(c.codigo, 'claimFeeReducido')} onChange={(e) => setCiudad(c.codigo, 'claimFeeReducido', e.target.value)} placeholder={`${empClaimRed}`} />}
                  </td>
