...
export default function Rutas() {
  ...
  const procesarArchivos = async (fileList) => {
    ...
    try {
      ...
      setReproMsg({ tipo: 'error', txt: t('No se pudo procesar:') + ' ' + err.message })
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-1 border-b border-slate-200 dark:border-slate-700/60">
        {[
          ...
          <button
            key={it.k}
            onClick={() => setTab(it.k)}
            className={`-mb-px inline-flex items-center gap-1.5 border-b-2 px-3 py-2 text-sm font-semibold transition ${tab === it.k ? 'border-brand-gold text-brand-navy dark:text-white' : 'border-transparent text-slate-500 hover:text-brand-navy dark:text-slate-400 dark:hover:text-white'}`}
          >
            <it.Icon size={15} strokeWidth={1.9} /> {t(it.l)}
          </button>
        ))}
      </div>
      ...
      {esDueno && tab === 'proyeccion' ? (
        ...
        <Cargando texto={t('Cargando rutas…')} />
      ) : !inv ? (
        <EstadoVacio titulo={t('Sin datos en este rango')} texto={t('No hay facturas en el rango seleccionado para analizar rutas.')} />
      ) : tab === 'precios' ? (
        !preciosPeso.hay ? (
          <Card className="p-4">
            ...
            <div className="mb-3 flex items-center gap-3">
              ...
              <div>
                <div className="font-semibold text-brand-navy dark:text-slate-100">{t('Sin desglose por peso todavía')}</div>
                <p className="m-0 text-sm text-slate-500 dark:text-slate-400">
                  {t('Esta factura no trae el precio por rango de peso (0-1lb, 1-5lb…).')}{' '}
                  {esDueno ? t('Reprocésala para extraerlo (solo alimenta Rutas y el simulador; no cambia pagos ni totales).') : t('Se guarda al cargar facturas nuevas.')}{' '}
                  {t('Mientras tanto, en')} <b>{t('Resumen')}</b> {t('ves el precio promedio por ruta.')}
                </p>
              </div>
            </div>
            {esDueno && (
              facturaUnica
                ? (
                  ...
                  >
                    ...
                    <div className="text-sm font-semibold text-brand-navy dark:text-slate-100">{reprocesando ? t('Procesando…') : t('Arrastra el Excel de esta factura aquí')}</div>
                    <div className="text-xs text-slate-400">{t('o haz clic para elegirlo · .xlsx, .xls')}</div>
                  </div>
                )
                : <span className="text-xs text-slate-400">{t('Elige arriba')} <b>{t('Una factura')}</b> {t('(una sola semana/ciudad) para poder reprocesarla.')}</span>
            )}
          </Card>
        ) : (
          <Card className="p-4">
              ...
              <table className="w-full min-w-[820px] border-collapse text-[13px]">
                <thead>
                  <tr className="bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                    ...
                    <th className="px-2.5 py-2.5 text-center font-semibold">{t('Dobles')}</th>
                    <th className="px-2.5 py-2.5 text-right font-semibold">{t('Ingreso ruta')}</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      {preciosPeso.rangos.map((rg) => {
                        ...
                        return (
                          <td key={rg} className="px-2 py-1.5 text-center">
                            ...
                            <div className="text-[11px] text-slate-400">{num(c.cantidad)} {t('paq')}</div>
                          </td>
                        )
                      })}
        ...
        <>
            ...
            <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {mejor && (
                <Card className="flex items-center gap-3 border-l-4 border-l-emerald-500 p-4">
                  ...
                  <div><div className="text-xs text-slate-400">{t('Ruta más rentable')}</div><div className="font-bold text-brand-navy dark:text-slate-100">{mejor.ruta} · {money(mejor.ganancia)}</div></div>
                </Card>
              )}
              {peor && (
                <Card className="flex items-center gap-3 border-l-4 border-l-rose-500 p-4">
                  ...
                  <div><div className="text-xs text-slate-400">{t('Ruta menos rentable')}</div><div className="font-bold text-brand-navy dark:text-slate-100">{peor.ruta} · {money(peor.ganancia)}</div></div>
                </Card>
              )}
            </div>
          )}
          ...
          <Card className="mb-3 p-3">
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative">
                ...
                <Input className="w-56 pl-8" value={busca} onChange={(e) => setBusca(e.target.value)} placeholder={t('Buscar ruta…')} />
              </div>
              <span className="ml-auto text-sm text-slate-500 dark:text-slate-400">{rows.length} {t('ruta(s)')}</span>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              <span className="mr-1 text-xs font-medium uppercase tracking-wide text-slate-400">{t('Ordenar:')}</span>
              {[
            ...
            <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
              <span className="font-medium text-slate-400">{t('Rentabilidad:')}</span>
              <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-emerald-500" /> {t('Más rentables')}</span>
              <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-amber-400" /> {t('Intermedias')}</span>
              <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-rose-500" /> {t('Menos rentables / pérdida')}</span>
            </div>
          </Card>
          ...
          <Card className="p-4">
            <div className="mb-2 flex items-center gap-2">
              ...
              <h3 className="m-0 text-base font-bold text-brand-navy dark:text-slate-100">{t('Todas las rutas')}</h3>
              <span className="ml-auto text-xs text-slate-400">{rows.length && rows.every((r) => r.costoExacto) ? t('Costo de choferes REAL (tarifa de cada chofer en la ruta)') : t('Costo de choferes estimado con la tarifa promedio (facturas nuevas traen el real)')}</span>
            </div>
            <div className="scroll-thin overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700/60">
              <table className="w-full min-w-[1000px] border-collapse text-[13.5px]">
                <thead>
                  <tr className="bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                    {cols.map((c) => (
                      <th key={c.k} onClick={() => cambiar(c.k)} className={`cursor-pointer whitespace-nowrap px-2.5 py-2.5 font-semibold ${c.txt ? 'text-left' : 'text-right'}`}>{t(c.label)}{flecha(c.k)}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  ...
                  {rows.length === 0 && <tr><td colSpan={cols.length} className="px-3 py-6 text-center text-slate-400">{t('Sin rutas con ese nombre.')}</td></tr>}
                </tbody>
              </table>
            </div>
