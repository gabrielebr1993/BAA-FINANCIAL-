...
export default function TrackingFicha() {
  ...
  return (
    <div>
      ...
      {rows === null ? (
        <Cargando texto={t('Buscando el tracking…')} />
      ) : rows.length === 0 ? (
        <EstadoVacio titulo={decoded || 'Tracking'} texto={t('Este paquete no tiene incidencias registradas. Solo guardamos los paquetes que tienen algún claim.')} mostrarBoton={false} />
      ) : (
        <>
          ...
          <Card className="mb-4 p-5">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="ml-auto text-right">
                <div className="text-[11px] uppercase tracking-wide text-slate-400">{t('Responsable')}</div>
          ...
          {/* Datos del paquete */}
          <Card className="mb-4 p-5">
            <h3 className="m-0 mb-3 text-base font-bold text-brand-navy dark:text-slate-100">{t('Datos del paquete')}</h3>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              <Dato icon={MapPin} label={t('Ruta')} valor={paquete?.ruta || '—'} />
              <Dato icon={MapPin} label={t('Ciudad')} valor={paquete?.ciudad ? nombreCiudad(paquete.ciudad) : '—'} />
          ...
          <Card className="p-5">
            <h3 className="m-0 mb-3 text-base font-bold text-brand-navy dark:text-slate-100">{t('Historial de incidencias')} ({historial.length})</h3>
            <div className="space-y-3">
              {historial.map((c) => {
                const estado = c.estadoRevision === 'anulado' ? { txt: t('Anulado'), color: 'slate' } : c.perdonado ? { txt: t('Perdonado'), color: 'green' } : c.estadoRevision === 'pendiente' ? { txt: t('Repetido · pendiente'), color: 'gold' } : { txt: t('Activo'), color: 'red' }
                return (
                      ...
                      <div className="ml-auto">
                        {perdonandoId === c.id ? (
                          <span className="inline-flex items-center gap-1.5">
                            <Input autoFocus className="w-40" placeholder={t('Motivo…')} value={motivo} onChange={(e) => setMotivo(e.target.value)} />
                          ...
                          <Boton variant="ghost" disabled={ocupado} onClick={() => restaurar(c)} className="px-2.5 py-1 text-xs">{t('Quitar perdón')}</Boton>
                        ) : c.estadoRevision === 'anulado' ? (
                          <span className="text-xs text-slate-400">{t('Sin acciones (anulado)')}</span>
                        ) : (
                          <Boton variant="ghost" onClick={() => { setPerdonandoId(c.id); setMotivo('') }} className="px-2.5 py-1 text-xs">{t('Perdonar')}</Boton>
                        )}
                      </div>
                    </div>
                    {c.perdonado && (
