...
export default function Usuarios() {
  ...
  const guardar = async () => {
    setError('')
    setOk('')
    if (!form.nombre.trim() || !form.email.trim()) return setError(t('Nombre y email son obligatorios.'))
    if (!activeCompanyId) return setError(t('No hay una empresa activa. Selecciona una empresa primero.'))
    const esDriver = form.role === 'driver'
    if (esDriver && !form.driverId) { setError(t('Elige a qué chofer se vincula este acceso.')); return }
    ...
    try {
      if (editId) {
        ...
        if (String(form.password).length < 6) return setError(t('La contraseña debe tener al menos 6 caracteres.'))
    ...
    <div>
      ...
      <Card className="mb-5 p-4">
        <div className="mb-3 flex flex-wrap items-center gap-3">
          <h3 className="m-0 text-base font-bold text-brand-navy dark:text-slate-100">{editId ? t('Editar usuario') : t('Crear usuario')}</h3>
          {!editId && (
            <label className="ml-auto flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
              ...
              {t('Modo manual (con UID, respaldo)')}
            </label>
          )}
        </div>
        <div className="mb-4 flex flex-wrap gap-3">
          ...
          {!editId && !modoManual && (
            <Campo label={t('Contraseña (mín. 6)')}>
              <Input className="w-52" type="password" value={form.password} onChange={(e) => setF('password', e.target.value)} placeholder="••••••••" />
            </Campo>
          )}
          {!editId && modoManual && (
            <Campo label={t('UID (Firebase Auth)')}>
              <Input className="w-52" value={form.uid} onChange={(e) => setF('uid', e.target.value)} placeholder={t('uid del usuario')} />
            </Campo>
          )}
          <Campo label={t('Rol')}>
            <Select className="w-40" value={form.role} onChange={(e) => setF('role', e.target.value)}>
              {ROLES.map((r) => (
                <option key={r} value={r}>{r === 'driver' ? t('driver (chofer)') : r}</option>
              ))}
            </Select>
          </Campo>
          {form.role === 'driver' && (
            <>
              <Campo label={t('Filtrar por ciudad')}>
          ...
          {form.role !== 'owner' && form.role !== 'driver' && (
            <Campo label={t('Ciudades asignadas')}>
              <div className="flex max-w-[520px] flex-wrap gap-1.5">
                ...
                {(ciudadesEmpresa || []).filter((c) => c.codigo).length === 0 && (
                  <span className="text-xs text-slate-400">{t('No hay ciudades configuradas. Ve a Configuración → Mis ciudades.')}</span>
                )}
              </div>
            </Campo>
          )}
        ...
        {form.role === 'driver' ? (
          <Aviso tipo="info">
            {t('Acceso de')} <b>{t('chofer')}</b>{t(': solo verá su portal (sus pagos, entregas, claims y calificación)')}{form.driverId ? <> {t('vinculado a')} <b>{driverNombreDe(form.driverId)}</b></> : ''}{t('. No ve finanzas ni a otros choferes ni el menú de gestión.')}
          </Aviso>
        ) : (
          <>
            <div className="mb-2 text-sm text-slate-500 dark:text-slate-400">
              {t('Permisos')} {form.role === 'owner' ? t('(el owner ve todo automáticamente)') : t('(elige qué puede ver)')}:
            </div>
            ...
            <Aviso tipo="info">
              {t('Este usuario podrá ver:')} {seccionesPermitidas.length === 0 ? t('ninguna sección.') : seccionesPermitidas.map((s) => s.label).join(', ') + '.'}
            </Aviso>
          </>
        )}
        ...
        <div className="flex gap-2">
          <Boton onClick={guardar} disabled={guardando} variant="gold">
            {guardando ? <><Spinner /> {editId ? t('Guardando…') : t('Creando…')}</> : editId ? t('Guardar cambios') : t('Crear usuario')}
          </Boton>
          {editId && <Boton onClick={nuevo} variant="ghost">{t('Cancelar')}</Boton>}
        </div>
        <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
          {editId
            ? t('Editar solo cambia el rol y los permisos del usuario (no su contraseña).')
            : modoManual
              ? t('Modo manual: crea solo el documento en users con un UID ya existente en Firebase Auth (respaldo si el servidor no está configurado).')
              : t('El sistema crea el acceso completo (correo + contraseña en Firebase Auth) y su documento, sin cerrar tu sesión. Requiere FIREBASE_SERVICE_ACCOUNT_BASE64 configurado en el servidor.')}
        </p>
      </Card>
      ...
      <Tabla
        ...
        emptyText={t('No hay usuarios registrados.')}
        renderCell={(row, key) => {
          ...
          if (key === 'permisos') {
            ...
            return activos.length ? (
              ...
              <span className="text-slate-400">{t('Sin permisos')}</span>
            )
          }
          if (key === 'acciones')
            return (
