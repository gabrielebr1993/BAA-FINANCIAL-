...
import { Card, Stat, PageTitle, Tabla, Aviso, Badge } from '../components/ui'
import Verificacion from '../components/Verificacion'
import CitySelector, { InvoiceSelector } from '../components/CitySelector'

export default function Dashboard() {
  const { invoices, selectedInvoice, selectedInvoiceId, claims, drivers, selectedCity } = useData()
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16, marginBottom: 18 }}>
        ...
        <RankCard titulo="Por calidad (menos claims = mejor)" lista={rc.calidad} valor={(p) => p.claimsTotales} fmt={num} invertido />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16 }}>
        <MiniLista
          titulo="Más reclamos"
          rows={rr.porClaims.filter((r) => (r.numClaims || 0) > 0).slice(0, 5)}
          render={(r) => `${r.ruta} — ${r.numClaims} claims`}
          vacio="Ninguna ruta con claims."
        />
        <MiniLista
          titulo="Cero reclamos"
          rows={rr.porClaims.filter((r) => (r.numClaims || 0) === 0).slice(0, 8)}
          render={(r) => `${r.ruta}`}
          vacio="Todas las rutas tienen algún claim."
        />
        <MiniLista
          titulo="Más ingreso"
          rows={rr.porIngreso.slice(0, 5)}
          render={(r) => `${r.ruta} — ${money(r.ingreso)}`}
        />
        <MiniLista
          titulo="Mejor $/lb"
          rows={rr.porPrecioLb.slice(0, 5)}
          render={(r) => `${r.ruta} — $${(r.precioPorLb || 0).toFixed(3)}/lb`}
        />
      </div>
    </div>
  )
...
// Ranking con barras horizontales + mejor/peor.
function RankCard({ titulo, lista, valor, fmt, invertido, marcaSinTarifa }) {
...
function Vacio() {
  return (
    <div>
      ...
      <Aviso tipo="info">
        Aún no hay facturas cargadas. Ve a <b>Cargar Factura</b> para subir el primer Excel de Gofo y ver aquí el resumen.
      </Aviso>
    </div>
  )
}
